...
public class FragmentStatMonth extends BaseFragment {

    @BindView(R.id.sm_start)
    TextView tv_start;
    @BindView(R.id.sm_stop)
    TextView tv_stop;
    @BindView(R.id.swipe_stat_month)
    SwipeRefreshLayout swipeRefreshStat;
    @BindView(R.id.rv_stat_month)
    RecyclerView rvStat;
    ...
    protected int getLayoutResource() {
        return R.layout.fragment_stat_month;
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        tv_start.setText(CommonUtils.firstMonthStrForYear());
        tv_stop.setText(CommonUtils.currentMonthStr());
        ...
        dayItemAdapter = new StatDayItemAdapter(mFragmentView.getContext(), true, false, dayInfo -> {
            Intent intent = new Intent(FragmentStatMonth.this.getActivity(), StatActivity.class);
            intent.putExtra("statType", "month");
    ...
    @OnClick({R.id.sm_start, R.id.sm_stop, R.id.sm_btn_stat})
    ...
    private void editDate(TextView textView) {
        try {
            ...
            DatePickerDialog pickerDialog = new DatePickerDialog(getContext(), (view1, year, month, dayOfMonth) -> {
                ...
                textView.setText(String.format("%s-%s", year, mon));
            }, y, m, d);
    ...
    private void initData() {
        curPage = 1;
        canLoad = true;
        if (dayInfoList != null && dayInfoList.size() > 0 && dayItemAdapter != null) {
    ...
    private void queryNextPageData() {
        if (canLoad && null != statPresenter & null != tv_start && null != tv_stop) {
            statPresenter.statMonth(curPage, pageSize, tv_start.getText().toString(), tv_stop.getText().toString());
        }
    }
...
//        initData();
    ...
    public void success(int processId, BaseResp result) {
        switch (processId) {
            case NetConstant.APP_MSG_STAT_MONTH:
