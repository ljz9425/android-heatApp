...
public class FragmentStatYear extends BaseFragment {

    @BindView(R.id.sy_start)
    TextView tv_start;
    @BindView(R.id.sy_stop)
    TextView tv_stop;
    @BindView(R.id.swipe_stat_year)
    SwipeRefreshLayout swipeRefreshStat;
    @BindView(R.id.rv_stat_year)
    RecyclerView rvStat;
    ...
    protected int getLayoutResource() {
        return R.layout.fragment_stat_year;
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        tv_start.setText(CommonUtils.currentYearStr());
        tv_stop.setText(CommonUtils.currentYearStr());
        ...
        dayItemAdapter = new StatDayItemAdapter(mFragmentView.getContext(), true, false, dayInfo -> {
            Intent intent = new Intent(FragmentStatYear.this.getActivity(), StatActivity.class);
            intent.putExtra("statType", "year");
    ...
    @OnClick({R.id.sy_start, R.id.sy_stop, R.id.sy_btn_stat})
    ...
    private void editDate(TextView textView) {
        try {
            ...
            DatePickerDialog pickerDialog = new DatePickerDialog(getContext(), (view1, year, month, dayOfMonth) -> {
               textView.setText(String.format("%s", year));
            }, y, m, d);
    ...
    private void queryNextPageData() {
        if (canLoad && null != statPresenter & null != tv_start && null != tv_stop) {
            statPresenter.statYear(curPage, pageSize, tv_start.getText().toString(), tv_stop.getText().toString());
        }
    }
...
//        initData();
    ...
    public void success(int processId, BaseResp result) {
        switch (processId) {
            case NetConstant.APP_MSG_STAT_YEAR:
